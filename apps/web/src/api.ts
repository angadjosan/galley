...
const urlToken: string | null = (() => {
  ...
  if (!token) return sessionStorage.getItem(URL_TOKEN_KEY);
...
export function linkIdFromLocation(): string | null {
 ...
 * the tab out.
 */
let renew: (() => Promise<string>) | null = null;
...
async function renewToken(): Promise<string | null> {
  ...
  if (!renewing) {
    ...
    void attempt.finally(() => {
      if (renewing === attempt) renewing = null;
    });
  }
  return renewing;
}
...
async function raw<T>(method: string, path: string, body?: unknown, authed = true): Promise<T> {
...
async function request<T>(method: string, path: string, body?: unknown): Promise<T> {
  try {
    ...
    if (!fresh) throw new ApiError(401, 'Your session ended. Sign in again to carry on.');
...
function toViewer(principal: RawPrincipal | undefined): Viewer {
  const kind = principal?.kind;
  return {
    id: principal?.id ?? principal?.principalId ?? 'me',
    kind:
      kind === 'agent' || kind === 'system' || kind === 'guest' ? kind : 'human',
...
export function devAuthEnabled(): boolean {
  return Boolean((window as { __GALLEY_DEV_AUTH__?: boolean }).__GALLEY_DEV_AUTH__);
}
...
interface ClerkLike {
  ...
  addListener?(fn: (state: { session?: unknown }) => void): (() => void) | void;
}

function clerk(): ClerkLike | null {
  return (window as { Clerk?: ClerkLike }).Clerk ?? null;
}
 ...
 */
export function getIdToken(email?: string, signal?: AbortSignal): Promise<string> {
  return devAuthEnabled() ? devIdToken(email) : clerkIdToken(signal);
}

async function devIdToken(email?: string): Promise<string> {
  const trimmed = email?.trim();
  if (!trimmed) throw new Error('Type the email address you want to sign in as.');
  return `dev:${trimmed}`;
}
...
async function clerkIdToken(signal?: AbortSignal): Promise<string> {
  ...
  if (!sso) {
    throw new Error(
      "Google sign-in isn't set up on this server yet. Whoever runs it needs to add Clerk — until then, a share link is the way in.",
    );
  }
  if (!sso.loaded && sso.load) await sso.load();

  if (!sso.session) {
    if (!sso.openSignIn) throw new Error('Google sign-in could not be opened. Reload and try again.');
    await new Promise<void>((resolve, reject) => {
      let settled = false;
      const stop = sso.addListener?.((state) => {
        if (settled || !state.session) return;
        settled = true;
  ...
  if (!proof) throw new Error('Google signed you in but did not hand over a token. Try once more.');
  return proof;
}

async function exchange(idToken: string): Promise<Viewer> {
  const result = await raw<{ token: string; principal?: RawPrincipal }>(
    'POST',
    '/v1/auth/session',
    { idToken },
    false,
  );
  if (!result.token) throw new ApiError(500, 'The server accepted the sign-in but returned no session.');
  token = result.token;
 ...
 * and asks Google.
 */
export async function signIn(email?: string, signal?: AbortSignal): Promise<Viewer> {
...
export async function signOut(): Promise<void> {
  try {
    if (token) await raw('POST', '/v1/auth/logout');
  } catch {
...
async function openLinkOnce(linkId: string): Promise<{ token: string; principal?: RawPrincipal; docId: string }> {
  return raw('POST', `/v1/links/${encodeURIComponent(linkId)}/open`, undefined, false);
}

export async function openLink(linkId: string): Promise<GuestSession> {
...
export type Boot =
  ...
  | { kind: 'user'; viewer: Viewer }
  | { kind: 'guest'; viewer: Viewer; docId: string; linkId: string };
...
export async function bootstrap(): Promise<Boot> {
  ...
  if (urlToken) {
    token = urlToken;
    try {
      const me = await raw<{ principal?: RawPrincipal }>('GET', '/v1/me');
  ...
  if (!devAuthEnabled()) {
    ...
    if (sso) {
      try {
        if (!sso.loaded && sso.load) await sso.load();
        if (sso.session) return { kind: 'user', viewer: await signIn() };
      } catch {
...
function capabilityOf(value: unknown): Capability {
...
export async function listAccess(ref: string): Promise<Access> {
  const result = await request<{ grants?: unknown[]; invites?: unknown[]; links?: unknown[] }>(
    'GET',
    `/v1/docs/${encodeURIComponent(ref)}/shares`,
  );
  const grants = (result.grants ?? []).map((entry) => {
    ...
    return {
      ...
      kind: (kind === 'agent' || kind === 'system' || kind === 'guest' ? kind : 'human') as PrincipalKind,
  ...
  const links = (result.links ?? []).map((entry) => toLink(entry as Record<string, unknown>));
  return { grants, invites, links };
}

function toLink(row: Record<string, unknown>): AccessLink {
...
export async function addShare(
  ...
  const result = await request<{ shared?: string }>('POST', `/v1/docs/${encodeURIComponent(ref)}/shares`, {
    email,
    capability,
  });
...
export async function removeShare(ref: string, principalId: string): Promise<void> {
...
export async function createLink(
  ...
  const result = await request<Record<string, unknown>>('POST', `/v1/docs/${encodeURIComponent(ref)}/links`, {
    capability,
    allowAgents,
  });
...
export async function listAgents(): Promise<AgentRow[]> {
  const result = await request<{ agents?: unknown[] }>('GET', '/v1/agents');
