...
export const suggestionKey = new PluginKey<readonly PendingSuggestion[]>('galley-suggestions');

export function suggestionReview(
  initial: readonly PendingSuggestion[],
  handlers: { current: SuggestionHandlers },
): Plugin<readonly PendingSuggestion[]> {
  return new Plugin<readonly PendingSuggestion[]>({
    key: suggestionKey,
    state: {
      init: () => initial,
      apply: (tr, value) =>
        (tr.getMeta(suggestionKey) as readonly PendingSuggestion[] | undefined) ?? value,
    },
    props: {
      decorations(state) {
        const pending = suggestionKey.getState(state);
        if (!pending || pending.length === 0) return DecorationSet.empty;
        ...
        state.doc.descendants((node, offset) => {
          ...
          decorations.push(
            Decoration.node(offset, offset + node.nodeSize, { class: 'block-suggested' }),
          );
          for (const suggestion of here) {
            const current = node.textContent;
            decorations.push(
              Decoration.widget(offset + node.nodeSize, () => renderCard(suggestion, current, handlers), {
                side: 1,
...
function renderCard(
  ...
  if (suggestion.state === 'stale') {
    ...
    actions.append(
      button('Use this', 'primary', () => handlers.current.accept(suggestion.id), suggestion.id),
      button('Dismiss', 'ghost', () => handlers.current.reject(suggestion.id)),
      button('Edit instead', 'ghost', () => handlers.current.acceptAndEdit(suggestion.id)),
    );
    foot.append(actions);
  }
  card.append(foot);
...
function button(label: string, kind: string, onClick: () => void, acceptId?: string): HTMLButtonElement {
  const element = document.createElement('button');
  element.type = 'button';
  element.className = kind;
  element.textContent = label;
  if (acceptId) element.setAttribute('data-testid', `accept-${acceptId}`);
...
export function diffWords(before: string, after: string): DiffPart[] {
  ...
  const push = (kind: DiffPart['kind'], text: string): void => {
    ...
    if (last && last.kind === kind) parts[parts.length - 1] = { kind, text: last.text + text };
