...
export function Stage(props) {
    ...
    const [camera, setCamera] = useState(IDENTITY);
    const [gesture, setGesture] = useState({ kind: 'none' });
     ...
     * drag implementation.
     */
    const travel = useRef({ x: { dir: 1, at: 0 }, y: { dir: 1, at: 0 } });
    /**
    ...
    const fitAll = useCallback(() => {
        ...
        const bounds = unionOf(props.design.frames.map((frame) => rects.get(frame.id)).filter((r) => !!r));
        if (!node || !bounds)
            return;
    ...
    useEffect(() => {
        if (fitted.current || rects.size === 0)
            return;
        fitted.current = true;
        fitAll();
    }, [fitAll, rects.size]);
    ...
    useEffect(() => {
        const node = viewport.current;
        const bounds = unionOf(props.selection.ids.map((id) => rects.get(id)).filter((r) => !!r));
        if (!node || !bounds || gestureRef.current.kind !== 'none')
            return;
        setCamera((current) => {
            const at = toViewport(current, bounds);
            const size = { width: bounds.width * current.zoom, height: bounds.height * current.zoom };
            const margin = 24;
            const slide = (start, extent, viewportExtent) => {
                if (start >= margin && start + extent <= viewportExtent - margin)
                    return 0;
                if (extent > viewportExtent - margin * 2)
                    return start - margin;
    ...
    useEffect(() => {
        const onDown = (event) => {
            ...
            if (event.key === 'Escape') {
                ...
                if (next)
                    latest.current.onSelection(next);
                else
            ...
            if (ARROWS.has(event.key)) {
                ...
                if (!id || !from || latest.current.readOnly)
                    return;
                ...
                if (index < 0 || index >= childCount(latest.current.design, from.parentId))
                    return;
            ...
            if (event.key === '=' || event.key === '+' || event.key === '-') {
                ...
                event.preventDefault();
                setCamera((current) => zoomAbout(current, centreOf(viewport.current), current.zoom * (event.key === '-' ? 1 / 1.2 : 1.2)));
            }
        };
        const onUp = (event) => {
            if (event.key === ' ')
    ...
    const onPointerDown = (event) => {
        ...
        if (event.button === 2) {
            ...
            if (claimed)
                props.onSelection({ focus: focusFor(props.design, claimed), ids: [claimed] });
            return;
        }
        if (event.button !== 0)
            return;
 ...
 * with tens of nodes.
 */
function resolveFor(design, rects, pointer, draggedId, zoom, travel) {
...
function updateTravel(travel, event) {
    for (const [axis, value] of [
        ...
        if (Math.abs(delta) < DIRECTION_WINDOW)
            continue;
