...
export function Overlay(props) {
    ...
    return (_jsxs("svg", { className: "design-overlay", "aria-hidden": "true", children: [focus && (
            ...
            _jsx("rect", { className: "design-overlay-focus", x: focus.x - 2, y: focus.y - 2, width: focus.width + 4, height: focus.height + 4 })), anchors.map((rect, index) => (_jsx("rect", { className: "design-overlay-anchor", x: rect.x, y: rect.y, width: rect.width, height: rect.height }, `anchor-${index}`))), hover && (_jsx("rect", { className: "design-overlay-hover", x: hover.x, y: hover.y, width: hover.width, height: hover.height })), selection.map((rect, index) => (_jsxs("g", { children: [_jsx("rect", { className: "design-overlay-selected", x: rect.x, y: rect.y, width: rect.width, height: rect.height }), selection.length === 1 && _jsx(Ticks, { rect: rect })] }, `selected-${index}`))), into && (_jsxs("g", { className: "design-overlay-into", children: [_jsx("rect", { x: into.rect.x, y: into.rect.y, width: into.rect.width, height: into.rect.height }), _jsxs("g", { transform: `translate(${into.rect.x + 4}, ${Math.max(12, into.rect.y - 6)})`, children: [_jsx("rect", { className: "design-overlay-tag-back", x: 0, y: -11, width: into.name.length * 6.2 + 12, height: 15, rx: 3 }), _jsx("text", { className: "design-overlay-tag", x: 6, y: 0, children: into.name })] })] })), line && (_jsx("line", { className: "design-overlay-drop", x1: toViewport(camera, { x: line.x1, y: line.y1 }).x, y1: toViewport(camera, { x: line.x1, y: line.y1 }).y, x2: toViewport(camera, { x: line.x2, y: line.y2 }).x, y2: toViewport(camera, { x: line.x2, y: line.y2 }).y })), marquee && (_jsx("rect", { className: "design-overlay-marquee", x: toViewport(camera, marquee).x, y: toViewport(camera, marquee).y, width: marquee.width * camera.zoom, height: marquee.height * camera.zoom }))] }));
}
