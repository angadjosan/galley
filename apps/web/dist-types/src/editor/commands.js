...
// ---------------------------------------------------------------------------
const mark = (id, label, name, shortcut) => ({
    id,
    label,
    shortcut,
...
export const CLEAR_FORMATTING = {
    id: 'clear',
    label: 'Clear formatting',
    shortcut: '⌘\\',
    command: clearFormatting,
};
export const UNDO = { id: 'undo', label: 'Undo', shortcut: '⌘Z', command: undo };
export const REDO = { id: 'redo', label: 'Redo', shortcut: '⌘⇧Z', command: redo };
// ---------------------------------------------------------------------------
    ...
    command: (state, dispatch) => {
        ...
        tr.doc.nodesBetween(from, to, (node, pos) => {
            if (node.type === schema.nodes.list_item)
                found.push({ pos, end: pos + node.nodeSize, checked: node.attrs.checked });
        });
        ...
        const items = found.filter((item) => !found.some((other) => other !== item && other.pos >= item.pos && other.end <= item.end));
        if (items.length === 0)
            return false;
        if (!dispatch)
            return true;
        ...
        for (const item of items) {
            ...
            if (node)
                tr.setNodeMarkup(item.pos, undefined, { ...node.attrs, checked: turningOn ? false : null });
        }
...
function wrapTransaction(state) {
    let captured = null;
 ...
 */
export function insertBlock(type, attrs) {
...
function insertNode(node) {
    return (state, dispatch) => {
        ...
        const atEmptyParagraph = empty && $from.parent.type === schema.nodes.paragraph && $from.parent.content.size === 0 && $from.depth === 1;
        const tr = state.tr;
        if (atEmptyParagraph) {
 ...
 */
export const INSERT_TABLE = (state, dispatch, view) => {
 ...
 */
export function insertDesignLink(path, label) {
    return (state, dispatch) => {
...
export function insertImage(src, alt) {
    return (state, dispatch) => {
        ...
        if (!image)
            return false;
        if (dispatch)
            dispatch(state.tr.replaceSelectionWith(image.create({ src, alt })).scrollIntoView());
        return true;
    };
}
// ---------------------------------------------------------------------------
 ...
 */
function bindingFor(shortcut) {
    const parts = [];
    let rest = shortcut;
    const take = (glyph, name) => {
