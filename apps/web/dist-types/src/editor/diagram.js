 ...
 */
export function resetDiagramTheme() {
    mermaidPromise = null;
    cache.clear();
...
export function onDiagramThemeChange(listener) {
...
export async function renderDiagram(lang, code) {
    ...
    try {
        ...
        const parsed = await mermaid.parse(trimmed, { suppressErrors: true }).catch(() => false);
        if (parsed === false) {
            return remember(key, {
                ok: false,
