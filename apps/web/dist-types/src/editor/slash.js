...
const insertNode = (type, attrs) => (state, dispatch) => {
...
export const slashKey = new PluginKey('galley-slash');
...
export function filterItems(query) {
    ...
    const scored = SLASH_ITEMS.map((item, order) => {
        ...
        if (label.startsWith(needle))
            score = 3;
        else if (item.aliases.some((alias) => alias.startsWith(needle)))
            score = 2;
...
function canOpen(state, from) {
    ...
    if ($from.parent.isAtom)
        return false;
    if (state.storedMarks?.some((mark) => mark.type.spec.code))
        return false;
...
export function slashPlugin(handlers) {
    const report = (view) => {
    ...
    return new Plugin({
        ...
        state: {
            ...
            apply(tr, previous) {
                ...
                if (meta?.type === 'open')
                    return { trigger: meta.trigger, query: '', index: 0 };
                if (meta?.type === 'close')
                    return null;
                if (!previous)
                    return null;
                if (meta?.type === 'move')
                    return { ...previous, index: meta.index };
                ...
                if (head < trigger + 1)
                    return null;
                if (selection.$from.before(selection.$from.depth) > trigger)
                    return null;
                if (tr.doc.textBetween(trigger, trigger + 1) !== '/')
                    return null;
                ...
                if (query.length > 6 && filterItems(query).length === 0)
                    return null;
 ...
 */
export function runSlashItem(view, item) {
    const state = slashKey.getState(view.state);
    if (state) {
...
export function closeSlash(view) {
    if (slashKey.getState(view.state)) {
        view.dispatch(view.state.tr.setMeta(slashKey, { type: 'close' }));
    }
}
/** Open the menu at the cursor, for the Insert button and the gutter `+`. */
export function openSlashAt(view, pos) {
